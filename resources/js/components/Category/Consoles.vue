<template>
    <div class="products-container">
      ...
      <div class="products-wrapper">
        <div class="card-product" v-for="product in consoles" :key="product.id">
...
export default {
  setup() {
    ...
    const fetchConsoles = async () => {
      try {
        const response = await axios.get('/api/categories/10/products')
  ...
  try {
    ...
    consoles.value = response.data.filter(product =>
          product.name.includes('Консоль')
        );

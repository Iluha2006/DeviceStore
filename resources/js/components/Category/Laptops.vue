<template>
    <div class="products-container">
      ...
      <div class="products-wrapper">
        <div class="card-product" v-for="product in laptops" :key="product.id">
...
export default {
  setup() {
    ...
    const fetchLaptops = async () => {
      try {
        const response = await axios.get('/api/categories/9/products')
  ...
  try {
    ...
    laptops.value = response.data.filter(product =>
          product.name.includes('Ноутбук')
        );

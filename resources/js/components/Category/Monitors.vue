<template>
    <div class="products-container">
      ...
      <div class="products-wrapper">
        <div class="card-product" v-for="product in monitors" :key="product.id">
...
export default {
  setup() {
    ...
    const fetchMonitors = async () => {
      try {
        const response = await axios.get('/api/categories/7/products')
  ...
  try {
    ...
    monitors.value = response.data.filter(product =>
          product.name.includes('Монитор')
        );
